...
public class GoodList01Adapter extends BaseQuickAdapter<GoodsListMoreModel.ListBeanX.ListBean, BaseViewHolder> {
    public GoodList01Adapter(int layoutResId, List<GoodsListMoreModel.ListBeanX.ListBean> data) {
        super(R.layout.good_listview_items, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, GoodsListMoreModel.ListBeanX.ListBean item) {
        Glide.with(mContext)
                .load(item.getThumb())
