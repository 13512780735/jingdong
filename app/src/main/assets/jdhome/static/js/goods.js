      ...
      if (data.code == 200 && data.data.list) {
        ...
        $.each(data.data.filtrate, function (i1, e1) {
          $.each(e1.list, function (i2, e2) {
            console.log(e2)
            sxi += `
          <span data-id="${e2.id}">${e2.name}</span>
        `
          })
          sxl += `
        <div class="gitem">
...
$('#selectBox .inner .btns .btn#confirm').click(function () {
  ...
  let sp = $('#selectBox .group .gitem .list span[class="on"]')
  let str = ''
  $.each(sp, function (i, e) {
    str += $(e).attr('data-id')
  })
  attribute = str.substr(0, str.length - 1);
  $('.goods .goodsGroup .list').html('')
  getData()

  // $('.top .type .item').removeClass('on')
  // $('.top .type .item[data-tag=type4]').addClass('on')
})
// 筛选关闭
